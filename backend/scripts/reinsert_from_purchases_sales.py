...
"""
Reinsert products from purchases and sales data - FIXED unique constraint
"""
...
def generate_unique_barcode(name, existing_barcodes, counter=0):
    """Generate a unique barcode"""
    base = name[:10].upper().replace(' ', '').replace('-', '')
    if not base:
        base = "PROD"
    ...
    if barcode in existing_barcodes:
        return generate_unique_barcode(name, existing_barcodes, counter + 1)
...
def combine_and_insert(purchase_products, sales_products):
        ...
        for row in sales_products:
            ...
            if name in product_map:
                ...
                if product_map[name]['cost'] == 0:
                    product_map[name]['cost'] = price * 0.7
    ...
    if conn:
        ...
        existing_barcodes = set([p[0] for p in existing if p[0]])
        cursor.close()
        conn.close()
    
    print(f"Existing barcodes in database: {len(existing_barcodes)}")
    
    # Prepare products with unique barcodes
    products_list = []
    used_barcodes = set(existing_barcodes)
    
    for name, data in product_map.items():
        # Generate unique barcode
        barcode = generate_unique_barcode(name, used_barcodes)
    ...
    print(f"📊 Unique barcodes generated: {len(df)}")
    ...
    print(df[['name', 'category', 'price', 'cost', 'stock', 'barcode']].head(20).to_string())
    
    # Show all products if requested
    show_all = input("\nShow all products? (yes/no): ").strip().lower()
    if show_all == 'yes':
        print(df[['name', 'category', 'price', 'cost', 'stock', 'barcode']].to_string())
    
    # Confirm import
    print("\n" + "-" * 70)
    response = input(f"\nDo you want to import {len(df)} products to inventory? (yes/no): ").strip().lower()
    ...
    clear_response = input("Clear existing products first? (yes/no): ").strip().lower()
    ...
    # Insert products with proper transaction handling
    inserted = 0
    skipped = 0
    errors = 0
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if inserted % 10 == 0:
                ...
                conn.commit()  # Commit every 10 to avoid large transactions
                
        except psycopg2.Error as e:
            if 'unique constraint' in str(e) or 'duplicate key' in str(e):
                print(f"  ⚠️ Duplicate barcode for {name}, generating new one...")
                # Generate a new unique barcode
                new_barcode = generate_unique_barcode(name, used_barcodes)
                used_barcodes.add(new_barcode)
                try:
                    cursor.execute("""
                        INSERT INTO products (barcode, name, category, price, cost, stock, reorder_level, branch_id)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    """, (new_barcode, name, category, price, cost, stock, reorder_level, 'HO'))
                    inserted += 1
                    print(f"  ✅ Added {name} with barcode {new_barcode}")
                    conn.commit()
    ...
    # Export backup
    export_response = input("\nExport products to CSV backup? (yes/no): ").strip().lower()
...
def main():
    ...
    if purchase_products or sales_products:
        ...
        print("   Your purchases and sales tables might be empty.")
