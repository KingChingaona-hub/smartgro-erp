...
def init_income():
    ...
    if not INCOME_FILE.exists():
        ...
        logger.info(f"Income file already exists: {INCOME_FILE} ({INCOME_FILE.stat().st_size} bytes)")
...
# ==============================
# SAVE - FIXED WITH BACKUP
# ==============================
def save_income(df):
    """Save income data with backup - NEVER DELETES"""
    try:
        if df is None:
...
def delete_income_by_id(date_str, income_source, amount, description=""):
    ...
    try:
        ...
        if INCOME_FILE.exists() and INCOME_FILE.stat().st_size > 0:
            try:
                shutil.copy2(INCOME_FILE, INCOME_BACKUP_FILE)
                logger.info(f"Created backup before deletion: {INCOME_BACKUP_FILE}")
            except Exception as e:
                logger.warning(f"Could not create backup: {e}")
