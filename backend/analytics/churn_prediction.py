...
def calculate_rfm_metrics(customer_transactions_df, sales_df, customers_df):
    ...
    for idx, customer in customers_df.iterrows():
        customer_name = customer.get(customer_col, "")
        customer_phone = customer.get(phone_col, "") if phone_col else ""
        ...
        if customer_sales.empty and phone_col and sales_df.get("customer_phone") is not None:
    ...
    for idx, customer in customers_df.iterrows():
        ...
        if not loyalty_df.empty:
            ...
            if not loyalty_data.empty:
                ...
                loyalty_tier = loyalty_data.iloc[0].get("tier", "BRONZE")
            ...
            if not customer_sales.empty:
                ...
                if len(customer_sales) > 1:
                    ...
                    if not date_diffs.empty:
                        purchase_regularity = date_diffs.std()
        ...
        if not sales_df.empty and sales_customer_col:
            ...
            if not customer_sales.empty and "payment_method" in customer_sales.columns:
                payment_methods = customer_sales["payment_method"].unique().tolist()
            ...
            if not customer_sales.empty and "barcode" in customer_sales.columns:
                ...
                if not products_df.empty and "barcode" in products_df.columns and "category" in products_df.columns:
                    ...
                    for _, sale in customer_sales.iterrows():
                        barcode = str(sale.get("barcode", ""))
                        product = products_df[products_df["barcode"].astype(str) == barcode]
                        if not product.empty:
                            category = product.iloc[0].get("category", "Unknown")
        ...
        features.append({
            "customer_name": customer_name,
            "phone": customer_phone,
            "recency_days": rfm_row.get("recency_days", 999),
            "frequency": rfm_row.get("frequency", 0),
            "monetary": rfm_row.get("monetary", 0),
            "avg_order_value": rfm_row.get("avg_order_value", 0),
            "loyalty_points": loyalty_points,
            "purchase_regularity": purchase_regularity,
            "tenure_days": tenure_days,
            "avg_items": avg_items,
            "payment_diversity": payment_diversity,
            "is_churned": rfm_row.get("is_churned", 1),
...
class ChurnPredictor:
    ...
    def prepare_data(self, features_df):
        ...
        if "favorite_category" in features_df.columns:
            self.feature_columns.append("favorite_category")
            le = LabelEncoder()
            features_df["favorite_category_encoded"] = le.fit_transform(
                features_df["favorite_category"].fillna("Unknown").astype(str)
            )
            self.label_encoders["favorite_category"] = le
            self.feature_columns.append("favorite_category_encoded")
        ...
        # Scale numeric features (excluding encoded categorical)
        numeric_cols = [
        ...
        scale_cols = [col for col in numeric_cols if col in X.columns]
...
def churn_prediction_dashboard():
    ...
    with tab1:
        ...
        if sales_date_col := get_date_column(sales_df):
            ...
            for _, customer in customers_df.iterrows():
                customer_name = customer.get(customer_col, "")
                customer_phone = customer.get(phone_col, "")
            ...
            if customer_col:
                ...
                if search_term:
                    ...
                    if not customer_results.empty:
                        ...
                        customer_name = selected_customer.get(customer_col, "")
