...
def generate_pl_pdf(pl_data, year=None, month=None, quarter=None):
    ...
    Args:
        ...
        quarter: Quarter for the report (optional)
    """
    buffer = BytesIO()
    ...
    transactions = pl_data.get('transactions', 0)
    items_sold = pl_data.get('items_sold', 0)
    avg_transaction = pl_data.get('avg_transaction', 0)
    ...
    net_sales = pl_data.get('net_sales', 0)
    operating_expenses = pl_data.get('operating_expenses', 0)
    expense_ratio = (operating_expenses / net_sales * 100) if net_sales > 0 else 0
    
    if expense_ratio > 40:
        insights.append(f"• High operating expense ratio of {expense_ratio:.1f}% - consider cost reduction")
    elif expense_ratio > 25:
        insights.append(f"• Moderate operating expense ratio of {expense_ratio:.1f}%")
    else:
        insights.append(f"• Low operating expense ratio of {expense_ratio:.1f}% - efficient operations")
...
def generate_profit_center_pdf(analysis_data, start_date=None, end_date=None):
    ...
    if analysis_data.get('top_products'):
        ...
        for product, data in list(analysis_data['top_products'].items())[:10]:
    ...
    if analysis_data.get('revenue_by_payment'):
        content.append(Paragraph("REVENUE BY PAYMENT METHOD", heading_style))
        content.append(Spacer(1, 6))
        
        payment_data = [['Payment Method', 'Revenue']]
        for method, revenue in analysis_data['revenue_by_payment'].items():
            payment_data.append([
                method,
                f"${revenue:,.2f}"
            ])
